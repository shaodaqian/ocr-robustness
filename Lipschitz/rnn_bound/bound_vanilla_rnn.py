...
class RNN(nn.Module):
    ...
    def extractWeight(self, clear_original_model=True):
        with torch.no_grad():
            ...
            # print(self.W_aa.size())
            # print(self.b_ax.size())
            # print(self.b_aa.size())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--work-dir', default = 'model/', type = str, metavar = 'WD',
    ...
    parser.add_argument('--eps0', default = 0.5, type = float,
    ...
    output = pickle.load(open("model/cnnout.pkl", "rb"))
    ...
    for i in range(31):
        ...
        weights = pickle.load(open("model/rnn.pkl", "rb"))
        rnn_state_dist=OrderedDict({'rnn.weight_ih_l0':torch.transpose(torch.tensor(weights[0]),0,1),
                                    'rnn.weight_hh_l0':torch.transpose(torch.tensor(weights[1]),0,1),
                                    'rnn.bias_ih_l0':torch.zeros(hidden_size, dtype=torch.float32),
                                    'rnn.bias_hh_l0':torch.tensor(weights[2]),
                                    'out.weight':torch.transpose(torch.tensor(weights[3]),0,1),
                                    'out.bias':torch.zeros(output_size,dtype=torch.float32)
                                    })
        rnn.load_state_dict(rnn_state_dist)
        rnn.to(device)
        print(rnn)
        cnnout=output[idx:idx+1,:time_step,:]
        out=rnn(cnnout)

        rnn.extractWeight(clear_original_model=False)
        target_label=torch.argsort(out)[0][-2]
        y=(torch.argmax(out))
        X=cnnout
        print('true',y)
        print('target',target_label)
        if y!=83:
            predlabel.append(CHAR_VECTOR[y-1])
        ...
        bounds[time_step]= [l_eps,u_eps,y,target_label]
    print(bounds)
    print(''.join(predlabel))
    # torch.save(bounds, save_dir+'certified_bound_%s'%(idx))
    w = csv.writer(open(save_dir+'certified_bound_%s.csv'%(idx), 'w'))
