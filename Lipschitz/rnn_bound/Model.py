...
class CRNN_model(object):
    ...
    def _build_model(self):
        ...
        drop2=layers.Dropout(0.3)(lstm2)
    ...
    def train(self,training_inputs, training_labels, val, val_split,
        ...
        model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
            'history/checkpoints/weights.{epoch:02d}-{val_loss:.2f}.hdf5', period=20)
...
class small_model(object):
    ...
    def _build_model(self):
        ...
        dp2=layers.Dropout(0.4)(conv6)
        conv7=layers.Conv2D(256, (3,3), padding='same', activation='relu')(dp2) #(batch,2,32,256)
        conv8=layers.Conv2D(512, (3,3), strides=(2,1), padding='same', activation='relu')(conv7) #(batch,1,32,512)
        bn1=layers.BatchNormalization()(conv8)
        dp3=layers.Dropout(0.4)(bn1)
        conv9=layers.Conv2D(512, (3,3), padding='same', activation='relu')(dp3)#(batch,1,32,512)
        bn2=layers.BatchNormalization()(conv9)
        dp4=layers.Dropout(0.4)(bn2)
        CNN_out=layers.Conv2D(512, (3,3), padding='same', activation='relu')(dp4) #(batch,1,32,512)

        reshape=layers.Reshape((32,512))(CNN_out) #(batch,32,512)
        ...
        rnn=layers.SimpleRNN(512, return_sequences=True)(RNN_in)
    ...
    def train(self,training_inputs, training_labels, val, val_split,
        ...
        # tensorboard_callback = keras.callbacks.TensorBoard(log_dir='tensor_board', histogram_freq=1, write_images=True)
        model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
            'history/small_checkpoints/weights.{epoch:02d}-{val_loss:.2f}.hdf5', period=20)
        early_stopping_checkpoint = keras.callbacks.EarlyStopping(monitor='loss', patience=5, verbose=1)
...
class smaller_model(object):
    ...
    def _build_model(self):
        ...
        self.model=keras.Model(inputs=input, outputs=[reshape,logits])
        self.model.compile(loss=self.loss.ctc_loss, optimizer=self.optimizer)#,metrics=[self.loss.ctc_beam_decoder_loss])
        # target_tensors = self.train_label
