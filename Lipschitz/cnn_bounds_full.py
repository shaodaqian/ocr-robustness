...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, InputLayer, Reshape
...
linear_bounds = None
from take_cnn import cnn_model
        ...
        while i < len(model.layers):
            ...
            if type(layer) == Conv2D:
                ...
                if padding == 'same':
                    ...
                    desired_w = int(np.ceil(cur_shape[1]/stride[1]))
                ...
                for k in range (len(a)):
                    self.weights[-1][k]=a[i]*self.weights[-1][k]
...
def find_output_bounds(weights, biases, shapes, pads, strides, sizes, types, x0, eps, p_n):
    ...
    for i in range(1,len(weights)+1):
        ...
        # print(math.sqrt(np.sum(np.square(LB))))
        # print(math.sqrt(np.sum(np.square(UB))))
        # print(math.sqrt(np.sum(np.square(np.subtract(LB, UB)))))
        UBs.append(UB)
        LBs.append(LB)
    return LBs[-1], UBs[-1]

def robustness_bound(LB,UB,preds):
    diff=np.zeros((LB.shape[0],LB.shape[1],LB.shape[2]),dtype=np.float32)
    diff2=np.zeros((LB.shape[0],LB.shape[1],LB.shape[2]),dtype=np.float32)
    for i in range (LB.shape[0]):
        for j in range (LB.shape[1]):
            for k in range (LB.shape[2]):
                if preds[i][j][k]!=0:
                    diff[i][j][k]=UB[i][j][k]-LB[i][j][k]
                    diff2[i][j][k]=(2*(UB[i][j][k]-LB[i][j][k])*preds[i][j][k])/(UB[i][j][k]+LB[i][j][k])
    lu=math.sqrt(np.sum(np.square(diff)))
    l=math.sqrt(np.sum(np.square(LB)))
    u=math.sqrt(np.sum(np.square(UB)))
    pre=math.sqrt(np.sum(np.square(preds)))
    a=(2*pre)/(l+u)
    return np.max(a*diff), np.max(diff2)


#Warms up numba functions
def warmup(model, x, eps_0, p_n, func,preds):
    print('Warming up...')
    weights = model.weights
    biases = model.biases
    shapes = model.shapes
    # W, b, s = model.weights[-1], model.biases[-1], model.shapes[-1]
    # last_weight = np.ascontiguousarray((W),dtype=np.float32)
    # weights.append(last_weight)
    # biases.append(np.asarray([b]))
    # shapes.append((1,31,256))
    LB, UB =func(weights, biases, shapes, model.pads, model.strides, model.sizes, model.types, x, eps_0, p_n)
    l=math.sqrt(np.sum(np.square(LB)))
    u=math.sqrt(np.sum(np.square(UB)))
    lu=math.sqrt(np.sum(np.square(np.subtract(LB, UB))))
    rob1,rob2=robustness_bound(LB,UB,preds)
    print('rob1',rob1)
    print('dff',UB-LB)
    # print('pred',preds)
...
def run(file_name, n_samples, p_n, q_n, activation = 'relu', cifar=False, tinyimagenet=False):
    ...
    keras_model = load_model('model/cnn.h5py', custom_objects={'fn':loss, 'tf':tf})
    model = Model(keras_model, inp_shape = (32,128,1))

    # cnn.model.load_weights(file_name, custom_objects={'fn':loss, 'tf':tf})
    # model = cnn.model
    ...
    print('preds',math.sqrt(np.sum(np.square(preds))))
    steps = 13
    eps_0 = 0.04
    summation = 0
    warmup(model, inputs[0].astype(np.float32), eps_0, p_n, find_output_bounds,preds)
    print(len(inputs))
        
    start_time = time.time()
    for i in range(len(inputs)):
        print('--- CNN-Cert: Computing eps for input image ' + str(i)+ '---')
        print('truelabel', text[idx])
        ...
        for j in range(steps):
            ...
            print("Step {}, eps = {:.5f}, rob={}".format(j,np.exp(log_eps),rob1))
            print('dff', UB - LB)
            if rob1 < tgtrob: #Increase eps
                log_eps_min = log_eps
        ...
        print("[L1] method = CNN-Cert-{}, model = {}, image no = {}, norm = {}, robustness = {:.5f}".format(activation,file_name, i,str_p_n,np.exp(log_eps_min)))
