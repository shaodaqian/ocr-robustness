...
class CompetitiveAlphaBeta:
    ...
    def target_pixels(self, image, pixels):
        ...
        labels,probabilities = self.MODEL.predict_all(manipulated_images)
    ...
    def play_game(self, image):
        ...
        for partitionID, pixels in self.PARTITIONS.items():
            ...
            sample=random.sample(pixels, 2)
            atomic_image=copy.deepcopy(image)
            for (x,y) in sample:
                print(image[x,y,0])
                if image[x,y,0]>0.5:
                    atomic = (x, y, 0, -1 * self.TAU)
                else:
                    atomic = (x, y, 0, 1 * self.TAU)
                valid, atomic_image = self.apply_atomic_manipulation(atomic_image, atomic)
            self.MODEL.save_input(atomic_image,"lb/comp/idx_%s_feature_%s.png" % (32,partitionID))
        ...
        #
        # print(self.BETA)
        # for partitionID, beta in self.MANI_BETA.items():
