...
class MCTSCooperative:
    ...
    def usefulAction(self, ampath, am):
        ...
        if (l1Distance(newConfident0,newConfident1)<0.002) and (linfDistance(newConfident0,newConfident1)<0.0006):
            # print('no')
            return False
        else:
            # print('yes')
    ...
    def initialiseExplorationNode(self, index, availableActions):
        ...
        if self.keypoint[index] != 0:
            count=0
            # samples = random.sample(availableActions[self.keypoint[index]].items(),300)
    ...
    #     cur = k
    #     new_move=[]
    #     path=self.atomicManipulationPath
    ...
    #             j+= 1
    #             if self.usefulAction(path,nextAtomicManipulation) == True:
    #                 break
    #         path=mergeTwoDicts(path,nextAtomicManipulation)
    #         orig=self.moves.applyManipulation(orig,nextAtomicManipulation)
    #         temp.append(orig)
    #         new_move.append(nextAtomicManipulation)
    ...
    #     temp=np.array(temp)
    #     # print(temp.shape)
    #     (newClasses,newconfident) = self.model.predict_all(temp)
    #
    ...
    #     for i in range(iterations):
    #         self.depth+=1
    #         dist = self.computeDistance(temp[i])
    #         self.atomicManipulationPath=mergeTwoDicts(self.atomicManipulationPath, new_move[i])
    #         if newClasses[i] != self.originalClass:
    ...
    #                 self.numConverge += 1
    #                 self.bestCase = (dist, self.atomicManipulationPath)
    #                 path0 = "ub/coop/%s_currentBest_%s.png" % (self.image_index, self.numConverge)
    #                 self.model.save_input(temp[i], path0)
    ...
    def sampleNext(self, k):
        ...
        while self.depth<1000:
            ...
            if newClass != self.originalClass:
                print("sampling a path ends in a terminal node with depth %s... " % self.depth)
                print('misclassified to',newClass)
                self.atomicManipulationPath = self.scrutinizePath(self.atomicManipulationPath)
                self.numAdv += 1
                newimg = self.moves.applyManipulation(self.image, self.atomicManipulationPath)
                dist = self.computeDistance(newimg)
                ...
                if self.bestCase[0] > dist:
                    ...
                    self.numConverge += 1
                    self.bestCase = (dist, self.atomicManipulationPath)
                    path0 = "ub/coop/%s_currentBest_%.2f_%s.png" % (self.image_index,dist, self.numConverge)
                    self.model.save_input(newimg, path0)
                    self.datalog.append(dist)
                return (self.depth == 0, dist)
